...
import { Building2, User, Globe, Info, Briefcase, Loader2, ArrowLeft } from "lucide-react";

export default function EditCompanyForm() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const id = searchParams.get("id");
  const { data, isLoading } = useSWR(id ? `/companies/${id}` : null, apiFetcher);
  ...
  if (isLoading) {
    return (
      <div className="min-h-[400px] flex flex-col items-center justify-center gap-4 card-meltwater bg-foreground/[0.02]">
        ...
        <p className="text-foreground/40 font-bold uppercase tracking-widest text-xs">Loading company details...</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="mb-8">
         <h1 className="text-3xl font-bold text-foreground tracking-tight mb-1">Edit Company</h1>
         <p className="text-foreground/40 text-sm font-medium">Update organization settings and contact information.</p>
      </div>
