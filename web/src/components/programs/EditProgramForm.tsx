...
import { Image as ImageIcon, Calendar, Info, Layout, Loader2, ArrowLeft } from "lucide-react";

export default function EditProgramForm() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const id = searchParams.get("id");
  const { data, isLoading: dataLoading } = useSWR(id ? `/programs/${id}` : null, apiFetcher);
    ...
    >
      <div className="mb-8">
         <div className="flex items-center gap-4 mb-4">
            ...
            <h1 className="text-3xl font-bold text-foreground tracking-tight">Edit Program</h1>
         </div>
         <p className="text-foreground/40 text-sm mt-1 font-medium">Update the program information and settings below.</p>
      </div>
             ...
             {uploadLoading ? (
                ...
                <div className="w-full p-6 flex flex-col items-center justify-center gap-4 bg-primary/5 rounded-2xl border border-primary/20 transition-all">
                   <div className="relative h-32 w-full max-w-md rounded-xl overflow-hidden shadow-lg border border-primary/10">
                      <img src={url || data?.image} alt="Program Banner" className="object-cover w-full h-full" />
                   </div>
                   <p className="text-xs font-bold text-primary uppercase tracking-widest">Banner Image Active</p>
