...
import { Mail, Calendar, Clock, ShieldCheck, MailCheck, Loader2, User as UserIcon, Activity } from "lucide-react";
  ...
  if (isLoading) {
    return (
      <div className="min-h-[400px] flex flex-col items-center justify-center gap-4 card-meltwater bg-foreground/[0.02]">
        ...
        <p className="text-foreground/40 font-bold uppercase tracking-widest text-xs">Loading Profile details...</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div className="lg:col-span-2 space-y-8">
        ...
        <div className="card-meltwater p-8">
          <div className="flex items-center gap-3 mb-8 pb-4 border-b border-border">
             ...
             <h3 className="text-xl font-bold text-foreground tracking-tight">Account Information</h3>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-10 gap-x-12">
            <div className="space-y-2">
              <span className="text-[10px] font-bold text-foreground/30 uppercase tracking-widest">Email Address</span>
            ...
            <div className="space-y-2">
              <span className="text-[10px] font-bold text-foreground/30 uppercase tracking-widest">Username</span>
              <div className="flex items-center gap-3">
                <div className="p-2 bg-foreground/5 rounded-lg">
                  <UserIcon size={16} className="text-primary" />
                </div>
                <span className="text-sm font-bold text-foreground">@{profile.email.split('@')[0]}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="card-meltwater p-8 bg-foreground/[0.01] border-dashed border-border flex flex-col items-center justify-center py-16 gap-4">
           ...
           <p className="text-sm font-bold text-foreground/20 uppercase tracking-widest">User Activity Logs Restricted</p>
        </div>
      </div>
      ...
      <div className="space-y-8">
        {/* Metadata Card */}
        <div className="card-meltwater p-8">
          <h3 className="text-lg font-bold text-foreground mb-8 tracking-tight">System Metadata</h3>
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-foreground">
                {profile.createdAt ? new Date(profile.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A'}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 text-xs font-bold text-foreground/40 uppercase tracking-widest">
              ...
              <span className="text-xs font-bold text-foreground">
                {profile.updatedAt ? new Date(profile.updatedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A'}
              </span>
            </div>
          </div>
        ...
        <div className="card-meltwater p-8 bg-primary/5 border-primary/10">
           <h4 className="text-sm font-bold text-foreground mb-4">Security Tip</h4>
           <p className="text-xs font-medium text-foreground/60 leading-relaxed">
              Always ensure the user has verified their email address before granting administrative privileges.
           </p>
        </div>
      </div>
    </div>
